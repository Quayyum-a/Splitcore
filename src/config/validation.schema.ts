...
export const validationSchema = Joi.object({
  ...
  DATABASE_URL: Joi.string()
    ...
    .required()
    .messages({
      'string.pattern.base': 'DATABASE_URL must be a valid PostgreSQL connection string starting with postgresql://',
  ...
  JWT_SECRET: Joi.string()
    ...
    .messages({
      ...
      'any.required': 'JWT_SECRET is required. Generate a secure random string (minimum 32 characters).',
    }),
  ...
  SENTRY_TRACES_SAMPLE_RATE: Joi.number()
    ...
    .default(0.1)
    .messages({
      'number.min': 'SENTRY_TRACES_SAMPLE_RATE must be between 0.0 (no tracing) and 1.0 (100% tracing)',
      'number.max': 'SENTRY_TRACES_SAMPLE_RATE must be between 0.0 (no tracing) and 1.0 (100% tracing)',
    }),
